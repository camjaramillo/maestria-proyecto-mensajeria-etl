...
def run_load(df: pd.DataFrame, target_session, truncate: bool = False) -> Tuple[bool, int]:
    ...
    try:
        # Eliminar tabla
        #target_session.execute(text("DROP TABLE IF EXISTS fact_servicio CASCADE"))
        ...
        """))
        if truncate:
            target_session.execute(text("TRUNCATE TABLE fact_servicio CASCADE"))
        df.to_sql(
            'fact_servicio',
        ...
        count = target_session.execute(text("SELECT COUNT(*) FROM fact_servicio")).scalar()
