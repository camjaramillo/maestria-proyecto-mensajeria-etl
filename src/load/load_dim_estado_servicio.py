...
def run_load(df: pd.DataFrame, target_session, truncate: bool = False) -> Tuple[bool, int]:
    ...
    try:
        # Eliminar tabla
        #target_session.execute(text("DROP TABLE IF EXISTS dim_estado_servicio CASCADE"))
        ...
        if truncate:
            target_session.execute(text("TRUNCATE TABLE dim_estado_servicio"))
        ...
        # Verificar conteo
        count = target_session.execute(text("SELECT COUNT(*) FROM dim_estado_servicio")).scalar()
